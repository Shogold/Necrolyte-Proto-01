...
var owenge = function ()
{
    ...
    var objectModel = [symbol.ombeg, symbol.omend];
    ...
    ns.equation.parse = function (s, om)
    {
        om = om || {};
        ...
        var recurse = function (s)
        {
            ...
            for (i = 0; i < len; i++)
            {
                ...
                if (ch == symbol.om)
                {
                    var omend = s.indexOf(symbol.om, i+1);
                    var omResults = readObjectModel(om, s.substring(i + 1, omend))
                    s = s.replace(s.substring(i, omend + 1), omResults);
                    i--;
                    continue;
                }
        ...
        var readObjectModel = function (objectModel, path)
        {
            var isFunc = false;
            var isEditableCollection = false;
            var recurse = false;
            var varName = path;
            var remainingPath = '';
            ...
            for (var i = 0; i < path.length; i++)
            {
                ...
                if (ch == symbol.prd)
                {
                    varName = path.substring(0, i);
                    recurse = true;
                    remainingPath = path.substr(i + 1);
                }
            }
            if (objectModel.constructor.toString() === 'EditableCollection')
            isEditableCollection = objectModel.constructor.toString() === 'EditableCollection';
            isFunc = objectModel[varName] && {}.toString.call(objectModel[varName]) === '[object Function]';
            var obj = isFunc ? objectModel[varName]() : objectModel[varName];

            return recurse ? readObjectModel(obj, remainingPath) : obj;
        }
